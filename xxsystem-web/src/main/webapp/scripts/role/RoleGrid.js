...
sshframe.role.addRole = function() {
	...
	// 设置默认值
	var roleTypeField = basicForm.findField('roleTypeId');
...
sshframe.role.deleteRole = function() {
	...
	for (var i = 0; i < rows.length; i++) {
		ids += (rows[i].data.roleId + ",");
	}
	ids = ids.substring(0, ids.length - 1);
	Ext.Msg.confirm(SystemConstant.alertTitle, "确认删除这" + rows.length + "条角色信息吗?", function(btn) {
		if (btn == 'yes') {
			Ext.Ajax.request({
