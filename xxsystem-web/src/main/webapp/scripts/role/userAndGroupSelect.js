		  ...
		  var userWin1 = Ext.create(Ext.window.Window,{
							...
							buttons:[{
								...
								handler:function(){
									...
									var params ;
									if(type =='scope'){
										ajaxUrl = basePath + '/role/addMeberUserScope.action';
										params = {roleMemeberId: roleId,userIds: userIds};
									}else{
										ajaxUrl = basePath + '/role/addRoleUser.action',
	...
	//建立groupStore
  	var groupStore1 = Ext.create('Ext.data.Store', {
		...
		proxy: {
			...
			url: basePath+'/group/getGroups.action',
			reader:{
				type: 'json',
	      			root: 'list',
	      			totalProperty:"totalSize"
	  ...
	  var groupWin = Ext.create(Ext.window.Window,{
						...
						buttons:[{
							...
							handler:function(){
								...
								Ext.Ajax.request({
									url : basePath + '/role/addRoleMember.action',
