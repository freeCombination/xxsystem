		   ...
		   items:[
		    			...
		    			readOnly:true,
		     			listeners : {
	  		     			"afterrender":function(com, eOpts){
			    				var startDateInput = Ext.getDom('taskStartTime-inputEl');
	   				   		startDateInput.initcfg = {dateFmt:'yyyy-MM-dd HH:mm:ss',disabledDates:[]};
	   				   		startDateInput.onclick = function(){
	   							WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'taskStartEndTime-inputEl\')}'});
	   						}
						   	}
							}
		    	     },
				   	...
				   	{	
						xtype:'textfield',
			    			...
			    			readOnly:true,
			     			listeners : {
		   		     			"afterrender":function(com, eOpts){
				    				var endDateInput = Ext.getDom('taskStartEndTime-inputEl');
				    				endDateInput.initcfg = {dateFmt:'yyyy-MM-dd HH:mm:ss',disabledDates:[]};
				    				endDateInput.onclick = function(){
		    							WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'taskStartTime-inputEl\')}'});
		    						}
							   	}
								}
			    	},
		       ...
		       {	
	    			...
	    			readOnly:true,
	     			listeners : {
 		     			"afterrender":function(com, eOpts){
		    				var startDateInput = Ext.getDom('taskEndStartTime-inputEl');
  				   		startDateInput.initcfg = {dateFmt:'yyyy-MM-dd HH:mm:ss',disabledDates:[]};
  				   		startDateInput.onclick = function(){
  							WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'taskEndTime-inputEl\')}'});
  						}
					   	}
						}
	    	     },
			   	...
			   	{	
					xtype:'textfield',
		    			...
		    			readOnly:true,
		     			listeners : {
	   		     			"afterrender":function(com, eOpts){
			    				var endDateInput = Ext.getDom('taskEndTime-inputEl');
			    				endDateInput.initcfg = {dateFmt:'yyyy-MM-dd HH:mm:ss',disabledDates:[]};
			    				endDateInput.onclick = function(){
	    							WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'taskEndStartTime-inputEl\')}'});
	    						}
						   	}
							}
		    	},
     ...
     columns: {
              items: [
                    {header:"序号",xtype: "rownumberer",width:60,align:"center",menuDisabled: true,sortable :false},
                          {header:'事项名称',dataIndex:'processName',menuDisabled : true,sortable:false,width:200,draggable:false,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
	     	                   /*  cellmeta.tdAttr = 'data-qtip="' + value + '"';
	     	                    return value; */
                        	  return  "<span title='"+value+"'>"+value+"</span>";
	     	          	}},
			  			 	{header:'类别',dataIndex:'processType',menuDisabled : true,sortable:false,width:200,draggable:false,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
			  			 		return  "<span title='"+value+"'>"+value+"</span>";
	     	          	}},
			  				{header:'任务名',dataIndex:'name',menuDisabled : true,sortable:false,width:150,draggable:false,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
			  					return  "<span title='"+value+"'>"+value+"</span>";
	     	          	}},
			  				...
			  				//{header:'承办人',dataIndex:'assigneeName',menuDisabled : true,sortable:false,width:150,draggable:false},
			  				{
								...
								items: [
						        	{
						                icon : '${ctx}/images/icons/preview.png', 
						                tooltip: '流程监控',
							...
							},
							{
								...
								items: [
						            {
						                icon : '${ctx}/images/icons/magnifier.png', 
						                tooltip: '详细信息',
						                handler: function(grid, rowIndex, colIndex) {
				 		                var row = grid.getStore().getAt(rowIndex);
				 		                var formUrl=row.get("formUrl");
			  				...
			  				{header:'创建日期',dataIndex:'createTime',
								...
								width:150,draggable:false,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
									return  "<span title='"+value+"'>"+value+"</span>";
		     	          	}
							},
							...
							{header:'耗时',dataIndex:'durationTime',draggable:false,
								...
								width:150,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
									var taskId = record.get('taskId');
		   							return '<img title="撤销审批" src="${ctx}/images/icons/bh.gif" style="cursor: pointer" onclick="dealshow('+taskId+');"/>';
		     	          	}
							}
              ]
        }
