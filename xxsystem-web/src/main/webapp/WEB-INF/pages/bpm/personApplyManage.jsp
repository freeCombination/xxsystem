...
var grid=Ext.create('Ext.grid.Panel', {
			...
			{	
				...
				readOnly:true,
					listeners : {
						...
						var startDateInput = Ext.getDom('processStartTime-inputEl');
				   		startDateInput.initcfg = {dateFmt:'yyyy-MM-dd',disabledDates:[]};
				   		startDateInput.onclick = function(){
							WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'processStartEndTime-inputEl\')}'});
						}
				   	}
					}
			 },
			   	...
			   	{	
					xtype:'textfield',
						...
						readOnly:true,
			 			listeners : {
				     			"afterrender":function(com, eOpts){
			    				var endDateInput = Ext.getDom('processStartEndTime-inputEl');
			    				endDateInput.initcfg = {dateFmt:'yyyy-MM-dd',disabledDates:[]};
			    				endDateInput.onclick = function(){
									WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'processStartTime-inputEl\')}'});
								}
						   	}
							}
				},
		       ...
		       {	
	    			...
	    			width:75,
	     			listeners : {
 		     			"afterrender":function(com, eOpts){
		    				var startDateInput = Ext.getDom('processEndStartTime-inputEl');
  				   		startDateInput.initcfg = {dateFmt:'yyyy-MM-dd',disabledDates:[]};
  				   		startDateInput.onclick = function(){
  							WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'processEndTime-inputEl\')}'});
  						}
					   	}
						}
	    	     },
			   	...
			   	{	
					xtype:'textfield',
		    			...
		    			readOnly:true,
		     			listeners : {
	   		     			"afterrender":function(com, eOpts){
			    				var endDateInput = Ext.getDom('processEndTime-inputEl');
			    				endDateInput.initcfg = {dateFmt:'yyyy-MM-dd',disabledDates:[]};
			    				endDateInput.onclick = function(){
	    							WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'processEndStartTime-inputEl\')}'});
	    						}
						   	}
							}
		    	}
     ...
     columns: {
              items: [       {header:"序号",xtype: "rownumberer",width:60,align:"center",menuDisabled: true,sortable :false,draggable:false,menuDisabled: true,sortable :false},   
                            {header:'事项名称',flex:2,dataIndex:'processName',sortable:true,draggable:false,menuDisabled: true,sortable :false,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
							...
							width:150,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
								if(record.get('processStatus')==0){
									var taskId = record.get('processInstanceId');
		   							return '<img title="重新发起" src="${ctx}/images/icons/bh.gif" style="cursor: pointer" onclick="dealshow('+taskId+');"/>';
								}else{
									return '';
								}
								}
