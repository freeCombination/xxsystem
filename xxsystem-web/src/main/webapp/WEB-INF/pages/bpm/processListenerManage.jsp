...
var grid=Ext.create('Ext.grid.Panel', {
     ...
     dockedItems:[{
					...
					xtype:"textfield",
						listeners :{
							"afterrender":function(com, eOpts ){
	               				var startTime = Ext.getDom('processStartTime-inputEl');
	               				startTime.initcfg={dateFmt:'yyyy-MM-dd',disabledDates:[]};
	               				startTime.onclick=function(){
									 WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'processStartEndTime-inputEl\')}',onpicked:function(){$dp.$('processStartTime-inputEl').focus();}});
	                            }
							}
						}
				},
			   	'到',{
					id:'processStartEndTime',
					width:80,
					xtype:"textfield",
						listeners :{
							"afterrender":function(com, eOpts ){
	               				var startTime = Ext.getDom('processStartEndTime-inputEl');
	               				startTime.initcfg={dateFmt:'yyyy-MM-dd',disabledDates:[]};
	               				startTime.onclick=function(){
									 WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'processStartTime-inputEl\')}',onpicked:function(){$dp.$('processStartEndTime-inputEl').focus();}});
	                            }
							}
						}
				}]
	     ...
	     xtype:"toolbar",
		   items:[
		       '结束时间',
		       {
					id:'processEndStartTime',
					width:80,
					xtype:"textfield",
						listeners :{
							"afterrender":function(com, eOpts ){
	               				var startTime = Ext.getDom('processEndStartTime-inputEl');
	               				startTime.initcfg={dateFmt:'yyyy-MM-dd',disabledDates:[]};
	               				startTime.onclick=function(){
									 WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'processEndTime-inputEl\')}',onpicked:function(){$dp.$('processEndStartTime-inputEl').focus();}});
	                            }
							}
						}
				},
			   	'到',
			   	{
					...
					xtype:"textfield",
						listeners :{
							"afterrender":function(com, eOpts ){
	               				var startTime = Ext.getDom('processEndTime-inputEl');
	               				startTime.initcfg={dateFmt:'yyyy-MM-dd',disabledDates:[]};
	               				startTime.onclick=function(){
									 WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'processEndStartTime-inputEl\')}',onpicked:function(){$dp.$('processEndTime-inputEl').focus();}});
	                            }
							}
						}
				},
     ...
     columns: {
              items: [       {header:"序号",xtype: "rownumberer",width:60,align:"center",menuDisabled: true,sortable :false},
                            {header:'事项名称',dataIndex:'processName',sortable:false,menuDisabled:true,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
							...
							{header:'发起时间',width:135,dataIndex:'startTime',sortable:false,menuDisabled:true,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
	     	                    cellmeta.tdAttr = 'data-qtip="' + value + '"';
	     	                    return value;
	     	          	}},
							{header:'结束时间',width:135,dataIndex:'endTime',sortable:false,menuDisabled:true,	 renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
	     	                    cellmeta.tdAttr = 'data-qtip="' + value + '"';
	     	                    return value;
	     	          	}},
