		...
		Ext.onReady(function() {
			...
			var informationStore=Ext.create("Ext.data.Store", {
				...
				proxy: {
				    ...
				    url: "${ctx}/group/getGroups.action",
				    reader: {
				...
				listeners:{
					'afterrender':function(){
						...
						for(var i = 0;i < userPermissionArr.length;i++){
							if("group_add_btn" == userPermissionArr[i].name){
								Ext.getCmp('addGroupBtn').setVisible(true);
							}
							if("group_update_btn" == userPermissionArr[i].name){
								Ext.getCmp('updateGroupBtn').setVisible(true);
							}
							if("group_delete_btn" == userPermissionArr[i].name){
				    	...
				    	handler: function(){
				    		if(groupForm.form.isValid()){
				    			groupForm.form.submit({
				    				url:"${ctx}/group/addGroupInfo.action",
				...
				var groupWin = Ext.create("Ext.window.Window",{
				    ...
				    buttons:[{
				    	...
				    	handler: function(){
				    		if(groupForm.form.isValid()){
				    			groupForm.form.submit({
				    				url:"${ctx}/group/updateGroupInfo.action",
			    				   	success : function(form, action) {
			...
			delGroupInfo = function(){
				...
				Ext.Ajax.request({
					url : '${ctx}/group/groupExsitInfo.action',
					params : {ids: ids},
					success : function(res, options) {
						 var data = Ext.decode(res.responseText);
						 var info = data.msg;
						 if(data.success){
							 Ext:Ext.Msg.confirm(SystemConstant.alertTitle,info,function(btn) {
									if (btn == 'yes') {
										Ext.Ajax.request({
											url : '${ctx}/group/delGroupInfo.action',
											params : {ids: ids},
											success : function(res, options) {
												Ext.getCmp("informationGrid").getSelectionModel().clearSelections();
...
/* 												informationStore.load(function(){
			...
			var groupMemberStore=Ext.create("Ext.data.Store", {
				...
				proxy: {
				    ...
				    url: "${ctx}/group/getGroupMembers.action",
				    reader: {
					     totalProperty: "totalSize",
					     root: "list"
				    },
			...
			var groupMemberGrid =  Ext.create("Ext.grid.Panel",{
				...
				listeners:{
					'afterrender':function(){
						for(var i = 0;i < userPermissionArr.length;i++){
							if("group_member_add_btn" == userPermissionArr[i].name){
								Ext.getCmp('addUserBtn').setVisible(true);
							}
							if("group_member_delete_btn" == userPermissionArr[i].name){
			...
			//groupMemberStore.load({params:{start:0,limit:10,groupId:groupId}});
...
// 				});
				...
				var userWin = Ext.create(Ext.window.Window,{
					...
					buttons:[{
						...
						handler:function(){
				    		...
				    		Ext.Ajax.request({
			                    url:"${ctx}/group/addGroupMember.action",
			                    method:'post',
				...
				//userStore.load({params:{start:0,limit:10,groupId:groupId}});
			}
			...
			delUser = function (groupId){
	    		...
	    		if(userIds != ""){
	    			Ext.Msg.confirm('系统提示','你确定要删除这'+rows.length+'条数据',function(btn){
	    				if(btn=='yes'){
	    					Ext.Ajax.request({
	    						url:'${ctx}/group/delGroupMember.action',
	    						method:'post',
