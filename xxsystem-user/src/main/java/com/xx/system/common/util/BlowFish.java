 ...
 * 数据加密，其加密算法采用SHA1
 * 
 * @version V1.20,2013-12-6 下午2:52:26
 * @see [相关类/方法]
 * @since V1.20
 * @depricated
 */
...
public class BlowFish {
    ...
    /** @Fields m_bfish : m_bfish */
    private BlowfishCBC m_bfish;
    
    /** @Fields m_rndGen : m_rndGen */
    private static Random m_rndGen = new Random();
    
    /** @Fields blowFish :blowFish */
    private final static BlowFish blowFish;
    
    static {
        blowFish = new BlowFish("r774UiWdqvL4d93");
    }
    ...
    public static String decrypt(String cipherText) {
        return blowFish.decryptString(cipherText);
    }
     ...
     * </p>
     * <p>
     * Description: Creates a new Blowfish object using the specified key (oversized password will
     * be cut)
     * </p>
     * 
     * @param password password the password (treated as a real unicode array)
     */
     ...
     * @Title encryptString
     * @author Markus Hahn <markus_hahn@gmx.net>
     * @Description: Encrypts a string (treated in UNICODE) using the standard Java random
     *               generator, which isn't that great for creating IVs
     * @date 2013-12-6
     * @param sPlainText sPlainText string to encrypt
     ...
     * @author Markus Hahn <markus_hahn@gmx.net>
     * @date 2013-12-6
     * @param sCipherText sCipherText hexbin string to decrypt
    ...
    public String decryptString(String sCipherText) {
        ...
        int nNumOfBytes =
            binHexToBytes(sCipherText, cbciv, 0, 0, BlowfishECB.BLOCKSIZE);
        ...
        nNumOfBytes =
            binHexToBytes(sCipherText, buf, BlowfishECB.BLOCKSIZE * 2, 0, nLen);
    ...
    private static class BlowfishECB {
        /** @Fields MAXKEYLENGTH : maximum possible key length */
        
        /** @Fields BLOCKSIZE : block size of this cipher (in bytes) */
        public final static int BLOCKSIZE = 8;
        
        /** @Fields PBOX_ENTRIES :size of the single boxes */
        final static int PBOX_ENTRIES = 18;
        
        /** @Fields SBOX_ENTRIES : SBOX_ENTRIES */
        final static int SBOX_ENTRIES = 256;
        
        /** @Fields m_pbox : m_pbox */
        int[] m_pbox;
        
        /** @Fields m_sbox1 : m_sbox1 */
        int[] m_sbox1;
        
        /** @Fields m_sbox2 : m_sbox2 */
        int[] m_sbox2;
        
        /** @Fields m_sbox3 : m_sbox3 */
        int[] m_sbox3;
        
        /** @Fields m_sbox4 : m_sbox4 */
        int[] m_sbox4;
        ...
        public static boolean selfTest() {
            ...
            byte[] testKey1 =
                {(byte)0x1c, (byte)0x58, (byte)0x7f, (byte)0x1c, (byte)0x13,
        ...
        protected long encryptBlock(long lPlainBlock) {
            ...
            nHi ^= pbox[0];
            nLo ^=
                (((sbox1[nHi >>> 24] + sbox2[(nHi >>> 16) & 0x0ff]) ^ sbox3[(nHi >>> 8) & 0x0ff]) + sbox4[nHi & 0x0ff])
                    ^ pbox[1];
            nHi ^=
                (((sbox1[nLo >>> 24] + sbox2[(nLo >>> 16) & 0x0ff]) ^ sbox3[(nLo >>> 8) & 0x0ff]) + sbox4[nLo & 0x0ff])
                    ^ pbox[2];
            nLo ^=
                (((sbox1[nHi >>> 24] + sbox2[(nHi >>> 16) & 0x0ff]) ^ sbox3[(nHi >>> 8) & 0x0ff]) + sbox4[nHi & 0x0ff])
                    ^ pbox[3];
        ...
        protected long decryptBlock(long lCipherBlock) {
            ...
            nHi ^= m_pbox[17];
            nLo ^=
                (((m_sbox1[nHi >>> 24] + m_sbox2[(nHi >>> 16) & 0x0ff]) ^ m_sbox3[(nHi >>> 8) & 0x0ff]) + m_sbox4[nHi & 0x0ff])
                    ^ m_pbox[16];
            nHi ^=
                (((m_sbox1[nLo >>> 24] + m_sbox2[(nLo >>> 16) & 0x0ff]) ^ m_sbox3[(nLo >>> 8) & 0x0ff]) + m_sbox4[nLo & 0x0ff])
                    ^ m_pbox[15];
            nLo ^=
                (((m_sbox1[nHi >>> 24] + m_sbox2[(nHi >>> 16) & 0x0ff]) ^ m_sbox3[(nHi >>> 8) & 0x0ff]) + m_sbox4[nHi & 0x0ff])
                    ^ m_pbox[14];
        ...
        /**
         * Encrypts a byte buffer (should be aligned to an 8 byte border) to another buffer (of the
         * same size or bigger)
         * 
         * @Title encrypt
         * @author Markus Hahn <markus_hahn@gmx.net>
        ...
        /**
         * encrypts an integer buffer (should be aligned to an two integer border) to another int
        ...
        /**
         * decrypts a byte buffer (should be aligned to an 8 byte border) to another byte buffer (of
        ...
        /**
         * decrypts an integer buffer (should be aligned to an two integer border) to another int
         * buffer (of the same size or bigger)
         * 
         * @param inbuffer buffer with ciphertext data
         * @param outbuffer buffer to get the plaintext data
    ...
    private static class BlowfishCBC extends BlowfishECB {
        ...
        /**
         * encrypts an int buffer (should be aligned to an two integer border) to another int buffer
         * (of the same size or bigger)
         * 
         * @param inbuffer buffer with plaintext data
         * @param outbuffer buffer to get the ciphertext data
         */
        ...
        /**
         * decrypts a byte buffer (should be aligned to an 8 byte border) to another buffer (of the
         * same size or bigger)
         * 
         * @param inbuffer buffer with ciphertext data
         * @param outbuffer buffer to get the plaintext data
